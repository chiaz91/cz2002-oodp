	...
	public void clearExpiredBookings() {
		...
		for (int i = bookings.size()-1; i >=0; i--) {
			...
			if (curBooking.getArrivalTime().before(dateExpired)) {
				count++;
				//System.out.println(String.format("[Test] BookingMgr.clearExpired: isExpired %s", curBooking));	
				System.out.println(String.format(curBooking.getName() + " has failed to arrive upon reservation time on " + curBooking.getArrivalTime().toGMTString() + "." + "Reservation will be cancelled."));
				this.bookings.remove(curBooking);
			}
			
		}
		if (count>0) {
			//System.out.println(String.format("[Test] BookingMgr.clearExpired: count(%d)", count));
			System.out.println(String.format("There are Total of " + count + " expired reservations.\n"));
		}
	}
